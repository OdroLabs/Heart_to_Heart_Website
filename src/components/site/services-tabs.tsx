              ...
              >
                <div className="flex items-center gap-3">
                  {service.icon && (() => {
                    ...
                    return IconCmp ? (
                      <span
                        className={`grid h-8 w-8 shrink-0 place-items-center rounded-xl ${isActive ? "bg-brand-50" : "bg-brand-50/60"
                          }`}
                      >
        ...
        <div className="relative overflow-hidden rounded-3xl shadow-md min-h-[320px] bg-brand-100">
          {active.image ? (
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <Icons.Heart className="h-16 w-16 text-primary/20" />
            </div>
          )}

          {/* Small floating badge showing service icon */}
          {active.icon && (() => {
            const ActiveIcon = (Icons as any)[active.icon];
            return ActiveIcon ? (
          ...
          <div className="mt-auto pt-6 space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {services.slice(0, 6).map((s_item) => (
                <span
                  key={s_item.id}
                  className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${s_item.id === active.id
